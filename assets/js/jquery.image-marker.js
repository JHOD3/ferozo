  ...
  'use strict';
    ...
    $.extend( Plugin.prototype, {
      ...
      addTextbox: function(marker) {
        ...
        var $title = $('<h2 class="image-marker__text-box__title" contenteditable="false">Title</h2>');
        ...
        /*
        var $btn_edit = $('<button type="button" class="btn">Edit</button>');
        $btn_edit.on('click', function() {
          //$(marker).draggable( "option", "disabled" );
        });
        $text_box.append($btn_edit);
        */
        if (!!marker.className) {
      ...
      mountTo: function($target, $text_box, $dot, $line, onDrag, marker) {
        ...
        if ($target.height() > $target.parent().height()) {
          $text_box.remove();
          return false;
          //$target.parent().parent().height( $target.parent().height() + 200 );
        }
        ...
        if (!this.settings.drag_disabled) {
          ...
          $close_btn.on('click', function() {
            ...
            marker.delete = true;
            //this._markers_delete.push($text_box.data('id'));
