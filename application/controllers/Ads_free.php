...
class Ads_free extends CI_Controller {

	private static $solapa = "ads_free";
	...
	public function view($prod_id = FALSE)
	{
		...
		$ads_imp_id = $this->ads_model->set_item_click($prod_id, NULL);
	...
	public function set_ads_impresion_ajax()
	{
		...
		{
			$ads_imp_id = $this->ads_model->set_item_impresion($this->input->post('ads_id'), NULL);
			if($ads_imp_id)
			{
				$data['error'] = FALSE;
				$data['data'] = "Impreso";
	...
	public function set_ads_click_ajax()
	{
		...
		if($this->input->post('ads_id') != "")
		{
			$ads_imp_id = $this->ads_model->set_item_click($this->input->post('ads_id'), NULL);
			if($ads_imp_id)
			{
				$data['error'] = FALSE;
				$data['data'] = "Impreso";
	...
	public function set_form_ajax()
	{
        ...
        {
			$ads_imp_id = $this->ads_model->set_item_form($this->input->post('ads_id'), NULL, $this->input->post('nombre'), $this->input->post('mail'), $this->input->post('telefono'), $this->input->post('consulta'));
			if($ads_imp_id)
			{
				$ads = $this->ads_model->get_items($this->session->userdata('idi_code'), $this->input->post('ads_id'));
				$not_id = $this->notificaciones_model->set_item(NULL, $ads['usr_id'], NULL, NULL, NOTIFICACION_ADS_NUEVO_FORM, NOTI_ESTADO_PENDIENTE, $ads_imp_id);
