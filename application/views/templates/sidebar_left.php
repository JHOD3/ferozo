	...
	<div class="sidebar-todo">
		<div class="panel panel-default" style="margin-bottom:0px;">
		    <div class="panel-body">
				...
				{
					echo "<a href='".site_url('user/profile')."'><img src='".base_url()."images/usuarios/".$this->session->userdata('usr_imagen')."' class='img-circle img-user' width='70px' style='margin-bottom: 10px;'></a><br>";
					
					$nombre_aux = "";
					if($this->session->userdata('usr_nombre') != "" && $this->session->userdata('usr_nombre') != "null")
					{
						...
						echo $this->session->userdata('usr_nombre');
						if($this->session->userdata('usr_apellido') != "" && $this->session->userdata('usr_apellido') != "null")
						{
							echo " ".$this->session->userdata('usr_apellido');
						}
						echo "</span>";
					}
					echo "<small style='width:100%; display:block; height:16px; overflow:hidden;'>".$this->session->userdata('usr_mail')."</small>";
		    	...
		    	foreach($tipos_producto as $tipo_prod)
		    	{
			    	...
			    	$cant = $this->productos_model->get_cant_items($this->session->userdata('usr_id'), $tipo_prod['tp_id']);
			    	$cant_num = 0;
			    	if($cant)
			    	{
			    		$cant_num = $cant['cant'];
		    	...
		    	$cant = $this->resultados_model->get_cant_items($this->session->userdata('usr_id'));
		    	$cant_num = 0;
		    	if($cant)
		    	{
		    		$cant_num = $cant['cant'];
		...
		<ul class="nav nav-pills nav-stacked">
	        ...
	        $active = "";
			if($this->router->fetch_class() == "cobranzas")
			{
				$active = "active";
			}
			echo '<li role="presentation" class="'.$active.'"><a href="'.site_url('cobranzas').'"><span class="fas fa-dollar-sign"></span> Cobranza documentaria</a></li>';
